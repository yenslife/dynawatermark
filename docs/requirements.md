...
1. 使用 ffprobe 讀取影片資訊。
2. 載入並驗證設定。
3. 根據影片資訊與設定產生 watermark events；random 模式自動產生，scheduled 模式使用指定事件。
4. 為每個 event 產生暫存 PNG，套用尺寸與透明度。
5. 使用 FFmpeg filter graph 將暫存 PNG 疊加到影片。
6. 輸出透明浮水印影片。
7. 使用同一批 events 產生紅色區塊人工核對版影片。
8. 產生 metadata JSON。

## 防盜與證明力限制

MVP 的重點是可見動態浮水印與事件紀錄，協助追蹤外流來源與人工核對位置。metadata 不包含 SHA-256，也不提供獨立 verify 指令。若未來需要法律證據鏈或檔案完整性驗證，可再加入私鑰簽章、可信時間戳記與檔案摘要。
